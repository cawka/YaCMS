	...
	*/
	function get_keys()
	{
		if (!empty($this->referer))
		{
			...
			else if (eregi('bing\.com', $this->referer))
			{
				// MSN
				preg_match("#{$this->sep}q=(.*?)\&#si", $this->referer, $this->keys);
			...
			else if (eregi('(search\.lycos\.com)', $this->referer))
			{
				// Lycos
				preg_match("#{$this->sep}query=(.*?)\&#si", $this->referer, $this->keys);
				$this->search_engine = 'Lycos';
			}
			else if (eregi('(altavista\.com)', $this->referer))
			{
				// AltaVista
				preg_match("#{$this->sep}q=(.*?)\&#si", $this->referer, $this->keys);
				$this->search_engine = 'AltaVista';
			}	
			else if (eregi('(duckduckgo\.com)', $this->referer))
			{
				// Duck Duck Go
				preg_match("#{$this->sep}q=(.*?)\&#si", $this->referer, $this->keys);
				$this->search_engine = 'Duck Duck Go';
			}	
			else if (eregi('(yebol\.com)', $this->referer))
			{
				// Yebol (China)
				preg_match("#key=(.*?)\&#", $this->referer, $this->keys);
				$this->search_engine = 'Yebol (China)';
			}
			else if (eregi('(search\.aol\.com)', $this->referer))
			{
				// AOL
				preg_match("#{$this->sep}q=(.*?)\&#si", $this->referer, $this->keys);
